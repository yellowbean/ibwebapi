(ns client-portal-web-api.api.market-data-beta-
...
(defn-spec hmds-history-get-with-http-info any?
  "Market Data History
  Using direct to market data farm connection, will provide a list of historical market data for a given conid"
  ([conid int?, period string?, ] (hmds-history-get-with-http-info conid period nil))
  ([conid int?, period string?, {:keys [bar outsideRth]} (s/map-of keyword? any?)]
   (check-required-params conid period)
   (call-api "/hmds/history" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"conid" conid "period" period "bar" bar "outsideRth" outsideRth }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    []})))

(defn-spec hmds-history-get history-result-spec
  "Market Data History
  Using direct to market data farm connection, will provide a list of historical market data for a given conid"
  ([conid int?, period string?, ] (hmds-history-get conid period nil))
  ([conid int?, period string?, optional-params any?]
   (let [res (:data (hmds-history-get-with-http-info conid period optional-params))]
     (if (:decode-models *api-context*)
        (st/decode history-result-spec res st/string-transformer)
        res))))
